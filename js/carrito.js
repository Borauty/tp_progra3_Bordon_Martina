...
//traigo los productos almacenados en localStorage como si fuera la bdd y voy bajando stock cuando vende
boton_confirmar_si.addEventListener("click", Event => {
    let carrito = traerCarrito();
    let productos = traerProductos();
    for (let i = 0; i < carrito.length; i++) {
        for (let index = 0; index < productos.length; index++) {
            if (carrito[i].id == productos[index].id) {
                productos[index].stock -= carrito[i].cantidad
                alert(productos[index].stock)
                break
            }
        }
    }
